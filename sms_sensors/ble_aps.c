 ...
 */
static uint32_t value_char_add(ble_aps_t * p_aps, const ble_aps_init_t * p_aps_init)
{
    ...
    ble_uuid.uuid = APS_UUID_VALUE_CHAR;
    ...
    return sd_ble_gatts_characteristic_add(p_aps->service_handle,
                                               &char_md,
                                               &attr_char_value,
                                               &p_aps->val_char_handles);
}
...
uint32_t ble_aps_init(ble_aps_t * p_aps, const ble_aps_init_t * p_aps_init)
{
    ...
    // Add characteristics.
    err_code = value_char_add(p_aps, p_aps_init);
    VERIFY_SUCCESS(err_code);
