...
#define APS_UUID_BUTTON_CHAR 0x1724
#define APS_UUID_LED_CHAR    0x1725
							  
// Forward declaration of the ble_aps_t type.
typedef struct ble_aps_s ble_aps_t;

typedef void (*ble_aps_val_write_handler_t) (ble_aps_t * p_imus, uint8_t pressure_value);
 ...
 *
 * @param[in] p_aps      Air Pressure Service structure.
 * @param[in] pressure_value  New pressure value.
 *
 * @retval NRF_SUCCESS If the notification was sent successfully. Otherwise, an error code is returned.
 */
uint32_t ble_aps_on_new_value(ble_aps_t * p_aps, uint8_t pressure_value);
